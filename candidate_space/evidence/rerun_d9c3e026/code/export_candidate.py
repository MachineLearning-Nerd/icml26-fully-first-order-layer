...
def main():
    ...
    runtime_line = next(
        (line for line in lines if line.startswith("REPRODUCTION_SUMMARY ")),
        None,
    )
    runtime = (
        json.loads(runtime_line[len("REPRODUCTION_SUMMARY ") :])
        if runtime_line
        else {
            "status": "BLOCKED",
            "terminal_reason": "claim verifier exited 2 before the cumulative summary",
            "claim_6_runtime_seconds": claim_6["runtime_seconds"],
        }
    )
    ...
    (destination / "negative_control_output.txt").write_text(
        ...
        + "\n"
        + json.dumps(runtime.get("negative_controls", {}), sort_keys=True)
        + "\n"
    )
